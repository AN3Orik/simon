	...
	protected void encodeBody(IoSession session, T message, IoBuffer out) {
		...
		try {
			...
			error.setErrorMessage(
					"Error while encoding interface lookup() return: Not able to write remote object name '" +
		...
		for (String class1 : interfaces) {
			try {
				...
				error.setEncodeError();
				error.setErrorMessage(
						"Error while encoding interface lookup() return: Not able to write interface name '" + class1 +
		...
		try {
			...
			MsgError error = new MsgError();
			error.setEncodeError();
			error.setErrorMessage("Error while encoding interface lookup() return: Not able to write errorMsg '" +
					message.getErrorMsg() + "' due to CharacterCodingException.");
			error.setRemoteObjectName(message.getRemoteObjectName());
			error.setInitSequenceId(message.getSequence());
			error.setThrowable(e);
			sendEncodingError(out, session, error);
