...
public class MsgInterfaceLookupEncoder<T extends MsgInterfaceLookup> extends AbstractMessageEncoder<T> {
	...
	protected void encodeBody(IoSession session, T message, IoBuffer out) {
		...
		log.trace("position before: {}", out.position());
		try {
			...
			error.setErrorMessage(
					"Error while encoding interface lookup() request: Not able to write interface name '" +
							message.getCanonicalInterfaceName() + "' due to CharacterCodingException.");
			error.setRemoteObjectName(null);
			error.setInitSequenceId(message.getSequence());
			error.setThrowable(e);
			sendEncodingError(out, session, error);
		}
		log.trace("position after: {}", out.position());
		log.trace("end");
	}
}
